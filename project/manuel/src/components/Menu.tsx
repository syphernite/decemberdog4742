...
type MenuItem = {
  ...
  name: string;
  description: string;
  price?: string;       // keep optional so you can fill later
  featured?: boolean;
};

// Master menu (edit freely)
const MENU: MenuItem[] = [
  // FRIED RICE
...
const Menu = () => {
  ...
  return (
    <section id="menu-page" className="relative bg-neutral-50">
      <div className="container mx-auto px-4 pt-24 md:pt-28 pb-16">
        {/* Card wrapper to match hero style (solid/glass-ready) */}
        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white shadow-2xl border border-black/5 ring-1 ring-black/5 p-6 md:p-10">
          {/* Title */}
          <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-6 md:mb-8">
            <h2 className="font-display text-5xl md:text-7xl font-normal text-black-deep tracking-tight">
              OUR MENU
            </h2>
            <p className="font-body text-xl text-gray-800 mt-3">
            ...
            <div className="inline-flex bg-white rounded-xl p-1 shadow border border-black/10">
              {(Object.keys(LABELS) as Category[]).map((c) => (
                <button
                  key={c}
                  onClick={() => setActive(c)}
                  className={`px-5 md:px-7 py-3 rounded-lg font-body font-bold text-sm md:text-base transition-all ${
                    active === c
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <div className="h-44 bg-gradient-to-br from-red-primary/5 to-red-primary/10 relative overflow-hidden">
                    {item.featured && (
                      <div className="absolute top-4 right-4 bg-red-primary text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1">
          ...
          >
            <motion.a
              ...
              className="bg-red-primary hover:bg-red-dark text-white px-8 py-4 rounded-xl font-body font-bold text-lg md:text-xl border-2 border-red-primary/70 inline-flex items-center justify-center gap-3"
