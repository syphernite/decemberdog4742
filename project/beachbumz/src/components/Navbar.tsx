...
// Place the image at: project/beachbumz/src/assets/beachbumz-logo.png
...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 ${scrolled ? 'bg-ocean-blue/95 backdrop-blur shadow-lg' : 'bg-ocean-blue/80 backdrop-blur'}`}
      style={{ height: '64px' }}
    >
      <div className="mx-auto max-w-6xl h-full px-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-2">
          {navItems.map(n => (
            <NavLink key={n.path} to={n.path} className={linkCls}>{n.label}</NavLink>
          ))}
      ...
      <div className={`md:hidden transition-[max-height] duration-300 overflow-hidden ${open ? 'max-h-80' : 'max-h-0'} bg-ocean-blue/98`}>
        <div className="px-4 pb-4 pt-2 grid gap-1">
          {navItems.map(n => (
            <NavLink key={n.path} to={n.path}
              className={({ isActive }) => `block rounded-md px-3 py-2 text-base ${isActive ? 'bg-white/10 text-turquoise' : 'text-white/90 hover:bg-white/10'}`}>
              {n.label}
            </NavLink>
          ))}
