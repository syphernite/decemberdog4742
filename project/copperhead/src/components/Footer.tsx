...
import { Facebook, Mail, Phone, Scissors } from 'lucide-react';
  ...
  return (
    <footer className="bg-ink border-t border-white/10">
      <div className="max-w-6xl mx-auto px-4 py-10 text-white/70 grid md:grid-cols-3 gap-6">
        ...
        <div className="text-sm">
          <div className="flex items-center gap-2">
            <Phone size={16}/> <a href="tel:+15805852072">580-585-2072</a>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <Mail size={16}/> <a href="mailto:info@copperheadcutz.com">info@copperheadcutz.com</a>
          </div>
        </div>
      ...
      <div className="text-center text-xs text-white/50 py-4 border-t border-white/10">
        © {new Date().getFullYear()} Copperhead Cutz
        <a
          href="https://built4you.org"
          target="_blank"
          rel="noreferrer"
          className="ml-2 inline-flex items-center gap-1 text-white/60 hover:text-white"
        >
          Made with <Scissors size={12}/> by Built4You
        </a>
      </div>
    </footer>
  )
