  ...
  return (
    <section className="min-h-[92svh] bg-ink text-bone">
      <div className="max-w-5xl mx-auto px-6 py-14">
        <h1 className="text-4xl md:text-5xl font-black copper-text">Prices</h1>
        <div className="mt-8 grid md:grid-cols-2 gap-4">
          {items.map((it) => (
            <div key={it.name} className="p-5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
              <div className="font-semibold text-white">{it.name}</div>
              <div className="text-white/80">${it.price.toFixed(2)}</div>
            </div>
          ))}
        </div>
